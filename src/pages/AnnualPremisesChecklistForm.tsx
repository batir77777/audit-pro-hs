...
import { AnnualPremisesChecklistReport, MonthlyPremisesChecklistItem, CorrectiveAction, ReportStatus } from '@/types';
import SectionHeader from '@/components/SectionHeader';
import StatusBadge from '@/components/StatusBadge';

const CHECKLIST_ITEMS = [
  "Ensure any fire alarm and emergency lights receive their 12-month service check by an external contractor.",
  "Ensure the fire-fighting equipment is serviced and checked by an external contractor.",
  "Have all portable electrical appliances been checked as safe (i.e. PAT.) (Test premises electrical systems 5 yearly.)",
  "Have gas, dust extraction, LEV and lifting equipment received their annual Certification e.g. LOLER?",
  "Are all essential H&S Training Certificates still valid e.g. First Aiders, Fire Wardens, CSCS, driver licences etc.?",
  "Have all risk assessments been reviewed and revised? (e.g. task, equipment, DSE, COSHH etc.)",
  "Have safety-critical documents been reviewed? (e.g. Fire FRA; Vehicles; H&S Policy; Asbestos; Legionella; Noise Surveys etc.)",
  "Have you analysed your last 12 months accident statistics with a view to making improvements?",
  "Have all corrective actions required from all previous health and safety Checklists been completed?",
  "Other vital checks"
];

export default function AnnualPremisesChecklistForm() {
  ...
  const [formData, setFormData] = React.useState<Omit<AnnualPremisesChecklistReport, 'id' | 'authorId' | 'status' | 'title' | 'type' | 'location'>>({
    premiseAddress: '',
    ref: '',
  ...
  const { photos, updatePhotos, clearPhotos } = usePhotoStore('annual_premises');
  const { clearAutoSave } = useAutoSave('annual_premises', formData);
  React.useEffect(() => {
    const saved = getAutoSavedData<any>('annual_premises'); // eslint-disable-line @typescript-eslint/no-explicit-any
  ...
  const handleSave = async (status: ReportStatus) => {
    ...
    try {
      ...
      const report: AnnualPremisesChecklistReport = {
        ...formData,
        id: newId,
        authorId: mockUser.id,
        status,
        title: `Annual Premises H&S Checklist - ${formData.date}`,
        type: 'Annual Premises Checklist',
      ...
      if (status === 'Submitted') {
        try {
          ...
          console.error('[AnnualPremisesChecklistForm] PDF generation failed:', pdfErr);
        }
      }

      navigate('/my-reports');
    } catch (err) {
      console.error('[AnnualPremisesChecklistForm] Save failed:', err);
    } finally {
  ...
  return (
    <div className="max-w-4xl mx-auto pb-20">
      ...
      <SectionHeader 
        title="5-003 Annual Premises H&S Checklist" 
        icon={ClipboardCheck}
        description="Annual premises and compliance review including servicing, training, and risk assessment reviews."
      />

      {/* Reminder Box */}
      <div className="mt-8 p-6 bg-sitk-yellow/10 border-2 border-sitk-yellow/20 rounded-[2rem] flex gap-4 items-start">
        ...
        <div className="space-y-1">
          ...
          <p className="text-sm font-medium text-slate-700 leading-relaxed">
            <span className="font-bold">REMEMBER:</span> This Annual H&S Checklist does not preclude the need for daily visual checks of the premises in relation to good health and safety.
          </p>
        </div>
      </div>
      ...
      <div className="mt-12 space-y-16">
        ...
        {/* Section 2: Annual Recorded Checks */}
        <section className="space-y-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-2xl font-black uppercase tracking-tight text-slate-900">Annual Recorded Checks</h2>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">Comprehensive annual health and safety review</p>
            </div>
          </div>
          ...
          <div className="mt-6 p-6 bg-slate-50 rounded-3xl border border-slate-100">
            <p className="text-[10px] font-bold text-slate-400 leading-relaxed uppercase tracking-widest">
              <span className="text-sitk-black">PAT</span> = Portable Appliance Testing; <span className="text-sitk-black">LEV</span> = Local Exhaust Ventilation; <span className="text-sitk-black">LOLER</span> = Lifting Operations and Lifting Equipment Regulations; <span className="text-sitk-black">CSCS</span> = Construction Skills Certification Scheme; <span className="text-sitk-black">DSE</span> = Display Screen Equipment; <span className="text-sitk-black">COSHH</span> = Control of Substances Hazardous to Health
            </p>
          </div>
        </section>
                  ...
                  <CardContent className="p-6 sm:p-8">
                    ...
                    <div className="grid gap-6 sm:grid-cols-3">
                      ...
                      <div className="space-y-2 sm:col-span-3">
                        <Label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Sign and date when completed</Label>
                        <Input 
          ...
          >
            ...
            <p className="text-slate-500 font-medium mb-8">
              {currentStatus === 'Draft' 
                ...
                : 'The annual premises checklist has been successfully recorded.'}
            </p>
