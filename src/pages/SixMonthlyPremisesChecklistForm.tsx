...
import { SixMonthlyPremisesChecklistReport, ChecklistItem, ReportStatus } from '@/types';
import SectionHeader from '@/components/SectionHeader';
import StatusBadge from '@/components/StatusBadge';

const CHECKLIST_ITEMS = [
  "Carry out a Fire Drill and evacuation and record the results in your Fire Logbook.",
  "Ensure any fire alarm and emergency lights receive their test or service check by an external contractor.",
  "Are the names of Fire Wardens and First Aiders still up-to-date and a list displayed?",
  "Are all risk assessments up-to-date and valid i.e. work tasks and equipment, DSE, premises etc?",
  "Have any new chemicals or substances been purchased and if so, are the COSHH assessments and training in place?",
  "Have lifting equipment and lifts been LOLER inspected?",
  "Have any new manual handling risks been assessed?",
  "Have all stepladders and ladders been inspected/recorded?",
  "Has storage and racking been checked/recorded as safe?",
  "Is all waste stored safely and disposed correctly?",
  "Are there other Six-Monthly Checklists to complete i.e. legionella, vehicles/transport, homeworkers, food etc.?",
  "Have all corrective actions required from the previous Checklist been completed?"
];

export default function SixMonthlyPremisesChecklistForm() {
  ...
  const [formData, setFormData] = React.useState<Omit<SixMonthlyPremisesChecklistReport, 'id' | 'authorId' | 'status' | 'title' | 'type' | 'location'>>({
    premiseAddress: '',
    ref: '',
  ...
  const { photos, updatePhotos, clearPhotos } = usePhotoStore('six_monthly_premises');
  ...
  const handleSave = async (status: ReportStatus) => {
    ...
    try {
      ...
      const report: SixMonthlyPremisesChecklistReport = {
        ...formData,
        id: newId,
        authorId: mockUser.id,
        status,
        title: `Six-Monthly Premises H&S Checklist - ${formData.date}`,
        type: 'Six-Monthly Premises Checklist',
      ...
      if (status === 'Submitted') {
        try {
          ...
          console.error('[SixMonthlyPremisesChecklistForm] PDF generation failed:', pdfErr);
        }
      }

      navigate('/my-reports');
    } catch (err) {
      console.error('[SixMonthlyPremisesChecklistForm] Save failed:', err);
    } finally {
  ...
  return (
    <div className="max-w-4xl mx-auto pb-20">
      ...
      <SectionHeader 
        title="5-002 Six-Monthly Premises H&S Checklist" 
        icon={ClipboardCheck}
        description="Six-monthly compliance and premises checks for health and safety."
      />
      ...
      <div className="mt-8 p-6 bg-sitk-yellow/10 border-2 border-sitk-yellow/20 rounded-[2rem] flex gap-4 items-start">
        ...
        <div className="space-y-1">
          ...
          <p className="text-sm font-medium text-slate-700 leading-relaxed">
            <span className="font-bold">REMEMBER:</span> This Six Monthly H&S Checklist does not preclude the need for daily visual checks of the premises and staff must report any obvious H&S hazards.
          </p>
        </div>
      </div>
      ...
      <div className="mt-12 space-y-16">
        ...
        {/* Section 2: Six Monthly Recorded Checks */}
        <section className="space-y-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-2xl font-black uppercase tracking-tight text-slate-900">Six Monthly Recorded Checks</h2>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">Regular compliance and premises verification</p>
            </div>
          </div>
                  ...
                  <CardContent className="p-6 sm:p-8">
                    ...
                    <div className="grid gap-6 sm:grid-cols-3">
                      ...
                      <div className="space-y-2 sm:col-span-3">
                        <Label className="text-[10px] font-black uppercase tracking-widest text-slate-400">By whom / by when</Label>
                        <Input 
                          value={action.byWhomWhen}
                          onChange={e => updateAction(action.id, 'byWhomWhen', e.target.value)}
        ...
        <div className="fixed inset-0 bg-sitk-black/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          ...
          >
            ...
            <p className="text-slate-500 font-medium mb-8">
              {currentStatus === 'Draft' 
                ? 'Your progress has been saved. You can complete this checklist later.' 
                : 'The six-monthly premises checklist has been successfully recorded.'}
            </p>
