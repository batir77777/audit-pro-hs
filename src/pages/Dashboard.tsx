...
import { getRecentActivity, getStoredReports } from '@/lib/mockData';
...
export default function Dashboard() {
  ...
  const [reports, setReports] = React.useState(getStoredReports);
    ...
    <div className="space-y-10">
      ...
      <section className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-1">
          ...
          <p className="text-muted-foreground font-medium text-sm">Safety is the Key Ltd | Site Safety Overview</p>
        </div>
      ...
      <div className="grid gap-8 lg:grid-cols-3">
        ...
        <Card className="border-none shadow-sm lg:col-span-2">
          ...
          <CardContent>
            <div className="space-y-1">
              {activities.length === 0 ? (
              <p className="text-sm text-muted-foreground font-medium px-4 py-6 text-center">No recent activity yet. Start by creating a report.</p>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm leading-tight">
                      <span className="font-black uppercase text-[10px] tracking-widest mr-2">{activity.userName}</span>
                    ...
                    <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-widest mt-1">{activity.timestamp}</p>
                  </div>
                </div>
              ))}
            </div>
            <Button variant="ghost" className="w-full mt-4 font-black uppercase text-[10px] tracking-widest hover:bg-sitk-yellow/10" onClick={() => navigate('/my-reports')}>
        ...
        <Card className="border-none shadow-sm">
          ...
          <CardContent>
            ...
            <Button className="w-full mt-6 bg-sitk-black text-white hover:bg-slate-800 font-black uppercase text-[10px] tracking-widest py-6" onClick={() => navigate('/my-reports')}>
              Manage All Reports
            </Button>
          </CardContent>
        </Card>
