...
import { PalletRackingChecklistReport, MonthlyPremisesChecklistItem, ReportStatus } from '@/types';
import SectionHeader from '@/components/SectionHeader';
import StatusBadge from '@/components/StatusBadge';

const CHECKLIST_ITEMS = [
  "Rack load signs displayed confirming the SWL?",
  "All the uprights and footplates intact and undamaged?",
  "Is the racking vertical?",
  "Are racking braces intact and undamaged?",
  "Are floor fixings intact and undamaged?",
  "Check that the beams are not overloaded. A large amount of beam deflection indicates overloading.",
  "Are beams intact, located correctly and undamaged?",
  "Are all beam connectors and safety clips in place?",
  "Are all welds intact and undamaged?",
  "Confirm no other visual signs of problems?"
];

export default function PalletRackingChecklistForm() {
  ...
  const [formData, setFormData] = React.useState<Omit<PalletRackingChecklistReport, 'id' | 'authorId' | 'status' | 'title' | 'type' | 'location'>>({
    premiseAddress: '',
    ref: '',
  ...
  const { photos, updatePhotos, clearPhotos } = usePhotoStore('pallet_racking');
  const { clearAutoSave } = useAutoSave('pallet_racking', formData);
  React.useEffect(() => {
    const saved = getAutoSavedData<any>('pallet_racking'); // eslint-disable-line @typescript-eslint/no-explicit-any
  ...
  const handleSave = async (status: ReportStatus) => {
    ...
    try {
      ...
      const report: PalletRackingChecklistReport = {
        ...formData,
        id: newId,
        authorId: mockUser.id,
        status,
        title: `Pallet Racking Monthly Checklist - ${formData.date}`,
        type: 'Pallet Racking Checklist',
      ...
      if (status === 'Submitted') {
        try {
          ...
          console.error('[PalletRackingChecklistForm] PDF generation failed:', pdfErr);
        }
      }

      navigate('/my-reports');
    } catch (err) {
      console.error('[PalletRackingChecklistForm] Save failed:', err);
    } finally {
  ...
  return (
    <div className="max-w-4xl mx-auto pb-20">
      ...
      <SectionHeader 
        title="5-017 Pallet Racking Monthly Checklist" 
        icon={Layers}
        description="Monthly safety inspection for industrial pallet racking systems."
      />
      ...
      <div className="mt-8 p-6 bg-sitk-yellow/10 border-2 border-sitk-yellow/20 rounded-[2rem] flex gap-4 items-start">
        ...
        <div className="space-y-1">
          ...
          <p className="text-sm font-medium text-slate-700 leading-relaxed">
            <span className="font-bold">REMEMBER:</span> Racking should be inspected monthly by nominated staff and annually by an independent person. If any racking has been damaged in any way, e.g. hit by a forklift, any unsafe components must be identified.
          </p>
        </div>
      </div>
        ...
        <section className="space-y-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">Regular structural and safety verification</p>
            </div>
          </div>
          ...
          >
            ...
            <p className="text-slate-500 font-medium mb-8">
              {currentStatus === 'Draft' 
                ...
                : 'The pallet racking monthly checklist has been successfully recorded.'}
            </p>
